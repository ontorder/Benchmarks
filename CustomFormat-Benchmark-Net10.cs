...
public class bench_tokenformat
{
    ...
    // TODO non potrebbe essere un InlineArray o che?
    ...
    [Benchmark]
    public string custom_format_pre()
    {
        var tokens = new TestTokens()
        {
            k0 = "userFirstName",
        ...
        return FormatStringWithPlaceholders_pre(Template, tokens, _temp);
    }
    ...
    private string FormatStringWithPlaceholders_pre(ReadOnlySpan<char> pattern, TestTokens placeholders, List<object> warnings)
    {
        if (pattern.Length == 0)
            return string.Empty;

        var positions = ArrayPool<short>.Shared.Rent(24);
        ...
        while (false == remainingPattern.IsEmpty)
        {
            ...
            if (openIndex < 0)
            {
                positions[lastPosition] = (short)PositionFlag.Final;
                break;
            }
            ...
            {
                case ['{', '{', ..]:
                    positions[lastPosition] = (short)PositionFlag.OpenEscapedGraffa;
                    positions[lastPosition + 1] = (short)openIndex;
                    remainingPattern = remainingPattern[2..];
                    continue;
                ...
                case ['{', ..]:
                    ...
                    switch (token)
                    {
                        case { } when placeholders.k0.SequenceEqual(token): tokenspace += placeholders.v0.Length - placeholders.k0.Length - 2; selected = 0; break;
                        case { } when placeholders.k1.SequenceEqual(token): tokenspace += placeholders.v1.Length - placeholders.k1.Length - 2; selected = 1; break;
                        case { } when placeholders.k2.SequenceEqual(token): tokenspace += placeholders.v2.Length - placeholders.k2.Length - 2; selected = 2; break;
                        case { } when placeholders.k3.SequenceEqual(token): tokenspace += placeholders.v3.Length - placeholders.k3.Length - 2; selected = 3; break;
                        case { } when placeholders.k4.SequenceEqual(token): tokenspace += placeholders.v4.Length - placeholders.k4.Length - 2; selected = 4; break;
                        case { } when placeholders.k5.SequenceEqual(token): tokenspace += placeholders.v5.Length - placeholders.k5.Length - 2; selected = 5; break;
                        default: warnings.Add("found unknown placeholder"); break;
                    }
                    positions[lastPosition] = (short)((short)PositionFlag.Placeholder | (selected << 8));
                    remainingPattern = remainingPattern[(closeIndex + 1)..];
                    break;

                case ['}', '}']:
                case ['}', '}', ..]:
                    positions[lastPosition] = (short)PositionFlag.ClosedEscapedGraffa;
                    remainingPattern = remainingPattern[2..];
                    break;
            ...
            // no check for max actions
            lastPosition += 2;
        }

        var formatted = string.Create(
            length: pattern.Length + tokenspace,
            state: new CreateStringState(placeholders, pattern, positions),
            static (dest, state) =>
            {
                var nMaxActions = state.Positions.Length / 2;
                var src = state.Src;
                var pos = state.Positions;
                var phs = state.Placeholders;
                var dst = dest;
                for (int posId = 0; posId < nMaxActions; ++posId)
                {
                    var posLocation = posId * 2;
                    var act = (PositionFlag)(pos[posLocation] & 0x0F);
                    switch (act)
                    {
                        case PositionFlag.Final:
                            src.CopyTo(dst);
                            return;

                        case PositionFlag.OpenEscapedGraffa:
                            dst[0] = '{';
                            src = src[2..];
                            dst = dst[1..];
                            break;

                        case PositionFlag.ClosedEscapedGraffa:
                            dst[0] = '}';
                            src = src[2..];
                            dst = dst[1..];
                            break;

                        case PositionFlag.Placeholder:
                            var tokenId = pos[posLocation] >> 8;
                            var lenToToken = pos[posLocation + 1];
                            src[..lenToToken].CopyTo(dst);
                            dst = dst[lenToToken..];
                            switch (tokenId)
                            {
                                case 0: phs.v0.CopyTo(dst); src = src[(lenToToken + 2 + phs.k0.Length)..]; dst = dst[phs.v0.Length..]; break;
                                case 1: phs.v1.CopyTo(dst); src = src[(lenToToken + 2 + phs.k1.Length)..]; dst = dst[phs.v1.Length..]; break;
                                case 2: phs.v2.CopyTo(dst); src = src[(lenToToken + 2 + phs.k2.Length)..]; dst = dst[phs.v2.Length..]; break;
                                case 3: phs.v3.CopyTo(dst); src = src[(lenToToken + 2 + phs.k3.Length)..]; dst = dst[phs.v3.Length..]; break;
                                case 4: phs.v4.CopyTo(dst); src = src[(lenToToken + 2 + phs.k4.Length)..]; dst = dst[phs.v4.Length..]; break;
                                case 5: phs.v5.CopyTo(dst); src = src[(lenToToken + 2 + phs.k5.Length)..]; dst = dst[phs.v5.Length..]; break;
                            }
                            break;
                    }
                }
        ...
        ArrayPool<short>.Shared.Return(positions);
        return new string(formatted);
    }

    enum PositionFlag : byte
    {
        Final = 1,
        OpenEscapedGraffa = 2,
        ClosedEscapedGraffa = 3,
        Placeholder = 4
    }

    ref struct CreateStringState
    {
        public TestTokens Placeholders;
        public short[] Positions;
        public ReadOnlySpan<char> Src;

        public CreateStringState(TestTokens placeholders, ReadOnlySpan<char> src, short[] positions)
        {
...
technically not 100% correct because of one last \0 byte surplus but close enough i guess
| Method              | Mean     | Error   | StdDev  | Gen0   | Gen1   | Allocated |
|-------------------- |---------:|--------:|--------:|-------:|-------:|----------:|
| custom_format_pre   | 653.1 ns | 7.46 ns | 6.61 ns | 0.9556 | 0.0105 |   5.86 KB |
| custom_format_stack | 721.7 ns | 6.20 ns | 4.84 ns | 1.3628 | 0.0277 |   8.35 KB |
| custom_format_list  | 749.8 ns | 3.00 ns | 2.81 ns | 1.3628 | 0.0277 |   8.35 KB |
*/
